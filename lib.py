...
def connect_db():
    try:
        ...
        conn.execute(
            """CREATE TABLE IF NOT EXISTS movie("id"	INTEGER,
	"title"	TEXT NOT NULL,
...
def import_movies():  # 將 JSON 資料插入到資料庫
    ...
    for movie in movies:
        cursor.execute(
            'INSERT INTO "movie" (title, director, genre, year, rating) VALUES (?, ?, ?, ?, ?)',
            (
                movie["title"],
                movie["director"],
                movie["genre"],
...
def search_movies():
    conn = sqlite3.connect("movie.db")  # 連線資料庫
    cursor = conn.cursor()  # 建立 cursor 物件
    search = input("查詢全部電影嗎？(y/n): ").strip().lower()
    ...
    if search == "y":
        ...
        print("\n")
        print(
            f"{'電影名稱':{chr(12288)}<10}{'導演':<10}{'  類型':>10}{'上映年份':>12}{'評分':>11}"
        )
        print(
            "------------------------------------------------------------------------"
        )
        ...
        for row in result_all:
            print(
                f"{row[1]:{chr(12288)}<10}{row[2]:{chr(12288)}<10}{row[3]:<10}{row[4]:<12}{row[5]:>11}"
            )
        ...
        print(
            f"{result_single[1]:{chr(12288)}<10}{result_single[2]:{chr(12288)}<10}{result_single[3]:<10}{result_single[4]:<12}{result_single[5]:<10.2}"
        )
...
def modify_movie():
    ...
    cursor.execute(
        "UPDATE movie SET title = ?, director = ?, genre = ?, year = ?, rating = ? WHERE title = ?",
        (
            change_title,
            change_director,
            change_genre,
...
def delete_movies():
    ...
    if delete == "y":
        ...
        movie_name = input("請輸入要刪除的電影名稱: ")

        # 查找是否存在該電影
        cursor.execute('SELECT * FROM "movie" WHERE title LIKE ?', (f"%{movie_name}%",))
        ...
        # 打印找到的電影信息
        print(
            f"{'電影名稱':{chr(12288)}<10}{'導演':<10}{'類型':<10}{'上映年份':<10}{'評分':<10}"
        )
        print(
            "------------------------------------------------------------------------"
        )
        for row in movies:
            print(
                f"{row[1]:{chr(12288)}<10}{row[2]:<10}{row[3]:<10}{row[4]:<10}{row[5]:<10}"
            )

        confirm = input(f"確定要刪除此電影嗎？(y/n): ")
        if confirm == "y":
            cursor.execute('DELETE FROM "movie" WHERE title LIKE ?', (f"%{movie_name}%",))
            conn.commit()
            print(f" 電影已刪除")
        else:
            print("操作已取消")
